...
var ComplianceStream  = function(options){
  ...
  // https://gnip-api.twitter.com/stream/compliance/accounts/{ACCOUNT_NAME}/publishers/twitter/{STREAM_LABEL}.json?partition=1

  var uri               = _.template("https://gnip-api.twitter.com/stream/compliance/accounts/<%- account_name %>/publishers/twitter/<%- stream_name %>.json", that.options);
      ...
      res.pipe(zlib.createGunzip())
          ...
          .pipe(eventStream.mapSync(function (data) {
            ...
            _.each(data.results, function(compliance_activity){
              ...
              //  See http://support.gnip.com/apis/compliance_firehose2.0/overview.html#ComplianceEvents for list of all compliance events
